...
class Trove:
    ...
    @trove.command(name="add", pass_context=True, hidden=True)
    async def add_(self, ctx, name: str, critical: bool, *, cont):
        ...
        if "[" in cont:
            cont = [cont.strip("\" ") for cont in cont[1:-1].split(",")]
        elif "," in cont:
            cont = [cont.strip("\" ") for cont in cont.split(",")]
        elif "\" \"" in cont:
            cont = [cont.strip("\" ") for cont in cont.split("\" \"")]
        if not self.permcheck(ctx):
            return
        elif not isinstance(critical, bool) or not isinstance(cont, list):
            await ctx.send("Please input the order correctly, i.e., [p]trove add \"Spring 2017\" True"
            ...
            if name not in troves:
                troves[name] = []
            counti = len(self.troves[name])
            for basket in self.troves[name]:
                if len(set(basket["items"]).intersection(cont)) >= 4:
                    if len(basket["items"]) < 8 and \
                            len(set(cont).difference(basket["items"])) > 0:
                        basket["items"].extend(list(set(cont).difference(basket["items"])))
                    basket["count"] += 1
                    await ctx.send("Count has been updated, basket now has a count of {}".format(basket["count"]))
                    return
                else:
                    counti -= 1
            if counti == 0:
                self.troves[name].append({"count": 1, "critical": critical, "items": cont})
                await ctx.send("Basket has been initialized, count is now 1")
        await self._trove.troves.set(troves)

    @trove.command(pass_context=True)
    async def simulate(self, ctx, *, name):
        ...
        trove_critical = "https://cdn.discordapp.com/attachments/302958104084217857/316185272918343680/crit.gif"
        description = "nyan"
        troves = await self._trove.troves()
        ...
        if name in troves2:
            ...
            await ctx.send("Trove not found, please make sure it's spelled correctly."
    ...
    @trove.group(pass_context=True, hidden=True)
    @checks.is_owner()
    ...
    async def __add(self, ctx, name: discord.Member=None):
        ...
        author = ctx.author
        trovekeeper = await self._troves.trovekeeper()
        if name is None:
            name = author
    ...
    @keeper.command(name="hackadd", pass_context=True)
    ...
    async def __remove(self, ctx, name: discord.Member=None):
        ...
        if len(trovekeeper) < 1:
            await ctx.send("trovekeeper list is currently empty, add new trovekeepers using points keeper add"
    ...
    async def __list(self, ctx):
        ...
        for x in trovekeeper:
            trovekeeper[trovekeeper.index(x)] = discord.utils.find(lambda N: N.id == x,
                                                                   self.bot.get_all_members()).display_name
        trovekeeper = sorted(trovekeeper, key=lambda item: (int(item.partition(' ')[0])
